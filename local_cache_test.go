...
func TestBuildInMapCache_Loop(t *testing.T) {
	cnt := 0
	c := NewMapCache(time.Second, BuildInMapCacheWithEvictedCallback(func(key string, val any) {
		cnt++
	}))
...
//	NewMapCache(time.Second, BuildInMapCacheWithEvictedCallback(func(key string, val any) {
//		// 刷新到数据库
//
//	}))
//}
